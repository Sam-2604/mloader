...
mloader is a single-file CLI wrapper. It has no classes — all logic is organised into standalone functions grouped by responsibility. The `main()` loop handles user input and orchestrates calls to the downloader functions.
...
`os.path.expanduser()` resolves the `~` home directory symbol correctly on macOS, Linux, and Windows. These are defined at the top of the file so they can be changed in one place without hunting through the code.
...
- **How it works:** `shutil.disk_usage(path)` returns total, used, and free bytes for the drive containing that path. Divides free bytes by `1024³` to get gigabytes.
...
- **First run flow:**
  ...
  3. Calls `validate_spotify_creds()` — loops back with an error message if rejected
...
- **When to use:** Wrong credentials were saved, or the Spotify Developer app was deleted and replaced.
...
- **How it works:** Calls spotdl as a subprocess with:
  - `download` — spotdl subcommand for downloading
  - `--format mp3` — enforce MP3 output
  - `--output output_path` — destination folder
  - `--client-id` / `--client-secret` — your personal Developer app credentials, bypassing the shared rate-limited key
- **Why subprocess instead of Python API:** spotdl has a Python API but it's less stable and more complex to integrate than its CLI, which is well-maintained and well-documented. Subprocess is simpler and more reliable here.
- **`check=False`:** Means a non-zero exit code from spotdl does not raise a Python exception — it's caught manually by checking `result.returncode`. This prevents one failed track from crashing the entire program.
- **Quality ceiling:** spotdl sources audio from YouTube/YouTube Music. Maximum quality is 256kbps regardless of what the Spotify track offers. This is a platform constraint — Spotify's servers are DRM-protected.
...
No state is carried between loop iterations. The CSV or database — mloader has no persistent storage beyond the credentials file. Every run reads the filesystem fresh.
...
- **No download resume** — if a large playlist download is interrupted (network drop, sleep, Ctrl+C), there is no resume state. yt-dlp will skip already-existing files on a re-run (it checks filenames). spotdl has its own cache for this but it's not explicitly managed by mloader.
